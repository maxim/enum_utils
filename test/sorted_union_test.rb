...
class SortedUnionTest < EnumUtilsTest
  def subject(*arrays, **kwargs)
    EnumUtils.sorted_union(*arrays.map(&:each), **kwargs)
  end

  def test_lazy
    enum1, tracker1 = make_lazy_enum('init', [1, 'a'], [2, 'b'], [4, 'c'])
    ...
    assert_equal(%w[c b], [tracker1, tracker2])
  ...
  def test_enums_without_index
    assert_equal [1,2,3,5], subject([1,2], [2,3], [1,2,5]).to_a
  end
